...
def inject_cookies(profile_dir, url, cookie_file):
    if not os.path.exists(cookie_file):
        print(f"[!] Error: File '{cookie_file}' nahi mili. Pehle apne Windows se cookies export kar ke VPS mein dalein.")
        return

    print(f"[*] Starting Chrome for {url} to inject cookies...")
    options = Options()
    ...
    try:
        ...
        print("[*] Refreshing page to save login state...")
        driver.refresh()
        time.sleep(5)
        print(f"[+] Done! Profile '{profile_dir}' is now permanently logged in on VPS.")
        driver.save_screenshot("cookie_success.png")
        print(">>> Saved 'cookie_success.png' to verify login. <<<")
...
if __name__ == "__main__":
    print("=========================================")
    print("=== Permanent Cookie Injector Tool ===")
    print("=========================================")
    print("1. Inject Facebook Cookies (from fb_cookies.json)")
    print("2. Inject Instagram Cookies (from ig_cookies.json)")
